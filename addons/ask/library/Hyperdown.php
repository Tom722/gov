...
class Hyperdown
{
     ...
     * @var string
     */
    public $_commonWhiteList = 'kbd|b|i|strong|em|sup|sub|br|code|del|a|hr|small|audio';

    /**
     * attr white list
     * @var string
     */
    public $_commonAttrWhiteList = 'title|src|href|id|class|style|width|height|alt|target|align|data-type|data-id|data-toggle|data-title|rel|controls|duration';
    /**
     * html tags
     *
     * @var string
    ...
    {
        $this->options = array_merge(get_addon_config('ask'), $options);
    }
    ...
    public function parseInline($text, $whiteList = '', $clearHolders = true, $enableAutoLink = true)
    {
        ...
        $text = preg_replace_callback(
            ...
            function ($matches) use ($self) {
                ...
                return $self->makeHolder(
                    "<a href=\"{$url}\" target=\"_blank\" {$nofollow}>{$link}</a>"
                );
            },
            $text
        );
        ...
        $text = preg_replace_callback(
            ...
            function ($matches) use ($self) {
                ...
                $url = isset($self->_definitions[$matches[2]]) ? $self->_definitions[$matches[2]] : '';
                $url = $self->makeImageUrl($url);
                $result = $url ?
                    "<img src=\"{$url}\" alt=\"{$escaped}\" title=\"{$escaped}\">"
                    : $escaped;
        ...
        $text = preg_replace_callback(
            ...
            function ($matches) use ($self) {
                ...
                $nofollow = '';
                $url = $this->makeUrl($url);
                return $self->makeHolder("<a href=\"{$url}\" target=\"_blank\" {$nofollow}>{$escaped}</a>");
            },
            $text
        );
        ...
        $text = preg_replace_callback(
            ...
            function ($matches) use ($self) {
                ...
                $nofollow = isset($self->_definitions[$matches[2]]) && stripos($self->_definitions[$matches[2]], request()->host()) !== false ? '' : ' rel="nofollow"';
                $result = isset($self->_definitions[$matches[2]]) ?
                    "<a href=\"{$self->_definitions[$matches[2]]}\"{$nofollow}>{$escaped}</a>"
                    : $escaped;
        ...
        if ($enableAutoLink) {
            $text = preg_replace_callback(
                ...
                function ($matches) use ($self) {
                    ...
                    $nofollow = '';
                    $url = $this->makeUrl($matches[2]);
                    return "{$matches[1]}<a href=\"{$url}\" target=\"_blank\" {$nofollow}>{$link}</a>{$matches[4]}";
                },
                $text
            );
        }
    ...
    {
        if (preg_match("/^(#+)(.*)$/", $line, $matches)) {
            if (preg_match("/(\d+)[\\\\]?\[(.*)[\\\\]?\]\(([a-zA-Z]+)\)/", $matches[2])) {
                return true;
            }
    ...
    protected function makeUrl($url)
    {
        ...
        if ($urlInfo && isset($urlInfo['host']) && $this->options['redirecturl']) {
            $url = str_replace("{url}", urlencode($url), $this->options['redirecturl']);
        }
        return $url;
    }
     ...
     * @return string
     */
    protected function makeImageUrl($url)
    {
        if (preg_match("/^\/uploads\//i", $url)) {
            return $url;
        }
        $allowList = explode(",", str_replace(["\r\n", "\r", "\n"], ",", $this->options['allowdomainlist']));
